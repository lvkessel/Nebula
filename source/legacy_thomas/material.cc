...
material_legacy_thomas::material_legacy_thomas(const std::string& name, double fermi, double barrier, double phonon_loss, double density) {
    _name = name;
    _fermi = fermi;
    _barrier = barrier;
...
material_legacy_thomas::material_legacy_thomas(const std::string& name, double fermi, double barrier, double phonon_loss, double band_gap, double density)
: material_legacy_thomas(name, fermi, barrier, phonon_loss, density) {
    _band_gap = band_gap;
}

double material_legacy_thomas::ionization_energy(double K, double P) const {
...
double material_legacy_thomas::outer_shell_ionization_energy(double omega0) const {
...
material_legacy_thomas& material_legacy_thomas::set_elastic_data(double K, const std::map<double,double>& dcs_map) {
...
material_legacy_thomas& material_legacy_thomas::set_inelastic_data(double K, const std::map<double,double>& dcs_map) {
    const double log_K = std::log(K);
    std::map<double,double> dcs_int_map;
    for(auto cit = dcs_map.cbegin(); cit != dcs_map.cend(); cit++) {
...
material_legacy_thomas& material_legacy_thomas::set_ionization_data(double B, const std::map<double,double>& tcs_map) {
...
material_legacy_thomas& material_legacy_thomas::set_outer_shell_ionization_data(const std::vector<double>& osi_vector) {
    _osi_energies = osi_vector;
    std::sort(_osi_energies.begin(), _osi_energies.end(), std::greater<double>());
    return *this;
}

archive::ostream& operator<<(archive::ostream& oa, const material_legacy_thomas& obj) {
...
archive::istream& operator>>(archive::istream& ia, material_legacy_thomas& obj) {
